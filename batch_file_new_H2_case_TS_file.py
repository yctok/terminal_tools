...
from omfit_classes import omfit_osborne
...
path="/sciclone/data10/mslaishram/solps-iter/runs/d3d/H2_g183508.05317_H_case_TSfile"
path_old1="/sciclone/data10/mslaishram/solps-iter/runs/d3d/H2_g183508.05317_H_case_TSfile/Attempt_0"
...
#pfile_loc = os.path.join(path_to_EXp_data,'p183766_03010')

#SHOT='183766.03010_906'
#pfile_loc= "/sciclone/data10/mslaishram/solps-iter/runs/d3d/gfiles_pfiles_datas/p{}".format(SHOT)

#pklfile_loc=os.path.join(path_to_EXp_data,'183766_3000_e8099.pkl')
...
f4 = open('Norm_iter_run_info.txt', 'w+')
...
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
    ...
    TS_ne =xp.data['pedData']['fitVals']['netanhpsi']['y']
    TS_nefit=calcTanhMulti(TS_ne,x_nefit)
    TS_nefit=1.0e20*TS_nefit    
    TS_nefit_SOL=calcTanhMulti(TS_ne,psinSOLPS)
    TS_nefit_SOL=1.0e20*TS_nefit_SOL 

    tefit = 1000*xp.data['pedData']['fitVals']['tedatpsi']['y']
    x_tefit = xp.data['pedData']['fitVals']['tedatpsi']['x']
    TS_te =xp.data['pedData']['fitVals']['tetanhpsi']['y']
    TS_tefit=calcTanhMulti(TS_te,x_tefit)
    TS_tefit=1000*TS_tefit    
    TS_tefit_SOL=calcTanhMulti(TS_te,psinSOLPS)
    TS_tefit_SOL=1000*TS_tefit_SOL 

           
    tifit = 1000*xp.data['pedData']['fitVals']['tidatpsi']['y']
    x_tifit = xp.data['pedData']['fitVals']['tidatpsi']['x']
    TS_ti =xp.data['pedData']['fitVals']['titanhpsi']['y']
    TS_tifit=calcTanhMulti(TS_ti, x_tifit)
    TS_tifit=1000*TS_tifit  
    TS_tifit_SOL=calcTanhMulti(TS_ti,psinSOLPS)
    TS_tifit_SOL=1000*TS_tifit_SOL 
    

    
    # # Calculation of residue
    
    res1_ne=TS_nefit_SOL-neSOLPS
    resT_ne=np.sqrt(np.sum(res1_ne**2)/len(res1_ne))
    #Normalized error
    res1_ne_sum=np.sum(res1_ne**2/TS_nefit_SOL**2)
    N_resT_ne=res1_ne_sum/(2*len(res1_ne))

    res1_te=TS_tefit_SOL-teSOLPS
    resT_te=np.sqrt(np.sum(res1_te**2)/len(res1_te))
    #Normalized error
    res1_te_sum=np.sum(res1_te**2/TS_tefit_SOL**2)
    N_resT_te=res1_te_sum/(2*len(res1_te))

    res1_ti=TS_tifit_SOL-tiSOLPS
    resT_ti=np.sqrt(np.sum(res1_ti**2)/len(res1_ti))  
    #Normalized error
    res1_ti_sum=np.sum(res1_ti**2/TS_tifit_SOL**2)
    N_resT_ti=res1_ti_sum/(2*len(res1_ti))
...
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
    ...
    if i >1:        
        if resT_ti < resT_ti_old and resT_ne < resT_ne_old :
            ...
            f4.write(str(i)+ ' ' + str(N_resT_te)+' ' + str(N_resT_ti)+ ' ' +str(N_resT_ne)+ '\n')
            resT_ne_old=resT_ne
            resT_te_old=resT_te
            resT_ti_old=resT_ti

            N_resT_ne_old=N_resT_ne
            N_resT_te_old=N_resT_te
            N_resT_ti_old=N_resT_ti                    
